...
PY_MAJOR = int(platform.python_version_tuple()[0])
...
class IP(object):
    ...
    def _dqtoi(self, dq):
        ...
        if ':' in dq:
            ...
            for h in hx:
                ...
                ip += h
            self.v = 6
            return long_func(ip, 16)
        elif len(dq) == 32:
            # Assume full heximal notation
            self.v = 6
            return long_func(h, 16)
        ...
        if '.' in dq:
            ...
            while len(q) < 4:
                q.insert(1, '0')
            self.v = 4
            return sum(long_func(byte) << 8 * index for index, byte in enumerate(q))
    ...
    def to_ipv4(self):
        ...
        if self.v == 4:
            ...
            if self.bin().startswith('0' * 96):
                return IP(long_func(self), version=4)
            elif self.bin().startswith('0' * 80 + '1' * 16):
                return IP(long_func(self) & MAX_IPV4, version=4)
            elif long_func(self) & SIX_TO_FOUR_MASK:
                return IP((long_func(self) - SIX_TO_FOUR_MASK) >> 80, version=4)
            else:
    ...
    def to_ipv6(self, ip_type='6-to-4'):
        ...
        if self.v == 4:
            if ip_type == '6-to-4':
                return IP(SIX_TO_FOUR_MASK | long_func(self) << 80, version=6)
            elif ip_type == 'compat':
                return IP(long_func(self), version=6)
            elif ip_type == 'mapped':
                return IP(0xffff << 32 | long_func(self), version=6)
        else:
            return self
...
class Network(IP):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            arr = list()
            while x < slice_stop:
                arr.append(IP(long_func(self) + x))
                x += slice_step
            return tuple(arr)
        else:
...
#
    for address, netmask, test_ips in tests:
        ...
        print('integer...: {0}'.format(long_func(net)))
